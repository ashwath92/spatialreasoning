...
class Prism(SpatialReasoning):
    ...
    def test(self, prob_type, num):
        ...
        NOTE: mod, which is returned by interpret/decide, contains intermediate
        models. Only the last model is sent to print_array_prism in case
        of single model problems. In multiple model problems, mods is not used,
        and the class variable self.neighbourhoodmodels (which is a
        defaultdict(list) ) is sent to print_array prism along with the
        threshold """
        prems = self.problems['combination'][num] if prob_type == 'combination' \
          ...
          else self.problems['generateall'][num] if prob_type == 'generateall'\
...
#        print(mods[len(mods)-1])
        if prob_type in ['generatedet', 'generateindet', 'generateall']:
            utilities.print_premises(prems[:-1])
        else:
            utilities.print_premises(prems)
        if len(self.modelslist) > 1:
            # Indicates an indeterminate problem: nothing is appended to
            # self.modelslist for a determinate problem.
            spatial_array.print_multiple_mods_prism(
                self.neighbourhoodmodels, self.threshold, prob_type,
                len(self.modelslist))
        else:
            spatial_array.print_array_prism(mods[len(mods)-1], prob_type)
    ...
    def interpret(self, prems):
        ...
        self.premises = prems
        self.intensionalrepr = [PARSER.parse(premise[0]) for premise in prems]
...
def main():
    ...
    try:
        ...
        prism = Prism()
        #super(Prism, prism).test(args.category, args.n)
        prism.test(args.category, args.n)
    except KeyError:
